  ...
  return (
    <div className="rounded-[2rem] border border-white/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,250,252,0.94))] p-2.5 shadow-[0_34px_90px_-54px_rgba(15,23,42,0.22)] backdrop-blur-xl dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.94),rgba(30,41,59,0.88),rgba(15,23,42,0.92))] dark:shadow-[0_34px_90px_-54px_rgba(15,23,42,0.58)]">
      <form
        action="/instructeurs"
        method="get"
        className="rounded-[1.7rem] border border-slate-200/80 bg-white/90 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] sm:p-3.5 dark:border-white/10 dark:bg-white/5 dark:shadow-none"
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 max-w-[18ch] text-[1rem] leading-[1.25] font-semibold text-slate-950 sm:max-w-none sm:text-[1.18rem] dark:text-white">
              Kies regio en voorkeuren in een rustige premium flow
            </p>
            <p className="mt-1 text-[12px] leading-5 text-slate-500 dark:text-slate-300">
              Begin met locatie en verfijn daarna je selectie zonder ruis.
            </p>
          </div>
          <div className="flex size-10 shrink-0 items-center justify-center rounded-[1rem] bg-primary/10 text-primary shadow-[0_14px_28px_-20px_rgba(37,99,235,0.18)] dark:bg-white/10 dark:text-sky-200 dark:shadow-[0_14px_28px_-20px_rgba(15,23,42,0.42)]">
          ...
          <div className="flex min-h-[3.2rem] items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <input
                ...
                placeholder="Bijvoorbeeld Rotterdam, Den Haag of Dordrecht"
                className="mt-0.5 w-full bg-transparent text-[14px] font-semibold text-slate-950 placeholder:font-medium placeholder:text-slate-400 focus:outline-none dark:text-white dark:placeholder:text-slate-500"
              />
            </div>
          </div>
        </label>
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          ...
          <SearchCardSelect
            ...
            onValueChange={(value) =>
              setFilters((current) => ({ ...current, rating: value }))
            }
            options={[
              { value: "0", label: "Alles" },
              { value: "4", label: "4.0+" },
        ...
        <div className="mt-3 flex flex-col items-start gap-1.5 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
          ...
          {hasActiveFilters ? (
            <button
              ...
              className="text-[11px] font-medium text-slate-500 transition-colors hover:text-slate-950 dark:text-slate-400 dark:hover:text-white"
            >
              Wissen
            </button>
          ) : null}
          ...
          {quickFilters.map((item) => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  setFilters((current) =>
                    active ? item.clear(current) : item.apply(current),
                  )
                }
                aria-pressed={active}
                className={`inline-flex min-h-9 items-center gap-2 rounded-full border px-3 py-1.5 text-[11px] font-medium transition-all duration-200 sm:text-[12px] ${
                  active
                    ? "scale-[1.02] border-sky-200 bg-sky-50 text-sky-900 shadow-[0_18px_34px_-20px_rgba(14,165,233,0.18)] dark:border-sky-300/24 dark:bg-sky-400/12 dark:text-sky-100"
                    : "border-slate-200 bg-white text-slate-700 hover:-translate-y-0.5 hover:border-slate-300 dark:border-white/10 dark:bg-white/6 dark:text-slate-200 dark:hover:border-white/16 dark:hover:bg-white/8"
                }`}
              >
                <span
                  className={`flex size-5 items-center justify-center rounded-full ${
                    active ? "bg-sky-100 dark:bg-sky-300/18" : "bg-slate-100 dark:bg-white/10"
                  }`}
                >
                  {active ? (
        ...
        <div className="mt-3 rounded-[1.2rem] border border-slate-200 bg-slate-50/90 px-4 py-3 shadow-[0_20px_40px_-30px_rgba(15,23,42,0.1)] dark:border-white/10 dark:bg-white/6 dark:shadow-[0_20px_40px_-30px_rgba(15,23,42,0.42)]">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5 sm:gap-2">
            {selectionSummary.map((item) => (
              <span
                key={item}
                className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[10px] font-medium text-slate-700 dark:border-white/10 dark:bg-white/6 dark:text-slate-200 sm:text-[11px]"
              >
                {item}
              </span>
            ))}
            {hasActiveFilters
              ? activeQuickFilters.map((item) => (
                  <span
                    key={item.label}
                    className="rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[10px] font-medium text-emerald-800 dark:border-emerald-300/24 dark:bg-emerald-400/12 dark:text-emerald-100 sm:text-[11px]"
                  >
                    {item.label}
                  </span>
                ))
        ...
        <div className="mt-3 rounded-[1.2rem] border border-slate-200 bg-slate-50/80 p-2 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.12)] dark:border-white/10 dark:bg-white/6 dark:shadow-[0_18px_40px_-34px_rgba(15,23,42,0.42)]">
          <div className="flex flex-col gap-2 md:flex-row">
            <Button
              type="submit"
              className="h-11 flex-1 rounded-full bg-[linear-gradient(135deg,#0f172a,#1d4ed8,#38bdf8)] text-white shadow-[0_22px_45px_-26px_rgba(37,99,235,0.38)] sm:h-10.5"
            >
              Zoek instructeurs
              <Search className="size-4" />
            </Button>
            <Button
              ...
              className="h-11 w-full rounded-full px-5 sm:h-10.5 sm:w-auto"
            >
              Filters wissen
            </Button>
          </div>
          <p className="mt-2 px-1 text-[11px] leading-5 text-slate-500 dark:text-slate-400 sm:text-[12px]">
            Je filters gaan direct mee naar de resultatenpagina, zodat je daar meteen in een
            rustige selectie terechtkomt.
          </p>
        </div>
      </form>
    </div>
