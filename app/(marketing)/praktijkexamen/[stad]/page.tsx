...
export async function generateMetadata({
  ...
  const intentConfig = getSeoCityIntentConfig("praktijkexamen");

  if (!city || !intentConfig) {
    return {
      title: "Praktijkexamen stadspagina | RijBasis",
    };
  }

  const canonicalPath = `/praktijkexamen/${city.slug}`;
  ...
  return {
    ...
    keywords: [
      ...
      `praktijkexamen training ${city.name}`,
      `laatste rijlessen ${city.name}`,
    ],
    alternates: {
...
export default async function PraktijkexamenCityPage({
  params,
}: {
  params: Promise<{ stad: string }>;
}) {
