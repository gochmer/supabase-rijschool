...
export async function generateMetadata({
  ...
  if (!city) {
    return {
      title: "Rijschool stadspagina | RijBasis",
    };
  }

  const metadata = getSeoCityVariantMetadata(city, "general");
...
export default async function RijschoolCityPage({
  params,
}: {
  params: Promise<{ stad: string }>;
}) {
  ...
  const instructors = await getPublicInstructorsByCity(city.slug, "auto");
