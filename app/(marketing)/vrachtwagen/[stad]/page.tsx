...
export async function generateMetadata({
  ...
  if (!city) {
    return {
      title: "Vrachtwagen rijles stadspagina | RijBasis",
    };
  }

  const metadata = getSeoCityVariantMetadata(city, "vrachtwagen");
...
export default async function VrachtwagenCityPage({
  params,
}: {
  params: Promise<{ stad: string }>;
}) {
  ...
  const instructors = await getPublicInstructorsByCity(city.slug, "vrachtwagen");
