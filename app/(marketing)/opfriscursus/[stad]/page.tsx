...
export async function generateMetadata({
  ...
  const intentConfig = getSeoCityIntentConfig("opfriscursus");

  if (!city || !intentConfig) {
    return {
      title: "Opfriscursus stadspagina | RijBasis",
    };
  }

  const canonicalPath = `/opfriscursus/${city.slug}`;
  ...
  return {
    ...
    keywords: [
      ...
      `opfris rijles ${city.name}`,
      `herstart rijles ${city.name}`,
    ],
    alternates: {
...
export default async function OpfriscursusCityPage({
  params,
}: {
  params: Promise<{ stad: string }>;
}) {
