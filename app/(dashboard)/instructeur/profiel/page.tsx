      ...
      <section className="relative overflow-hidden rounded-[2.2rem] border border-white/70 bg-[linear-gradient(135deg,#020617,#172554,#0284c7)] p-6 text-white shadow-[0_34px_110px_-58px_rgba(15,23,42,0.78)] dark:border-white/10 sm:p-7">
        ...
        <div className="relative grid gap-7 xl:grid-cols-[1fr_auto] xl:items-end">
          ...
          <div className="grid min-w-[17rem] gap-3 rounded-[1.6rem] border border-white/14 bg-white/10 p-4 backdrop-blur">
            ...
            <p className="text-4xl font-semibold">{safeScore}%</p>
            ...
            <p className="text-xs leading-5 text-white/68">
              Hoe completer je profiel, hoe sterker je openbare pagina converteert.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="space-y-5">
          <section className="rounded-[1.8rem] border border-white/70 bg-white/90 p-5 shadow-[0_26px_86px_-48px_rgba(15,23,42,0.38)] dark:border-white/10 dark:bg-[linear-gradient(145deg,rgba(15,23,42,0.92),rgba(30,41,59,0.86),rgba(15,23,42,0.94))]">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-semibold tracking-[0.2em] text-primary uppercase">Live preview</p>
                <h2 className="mt-1 text-xl font-semibold text-slate-950 dark:text-white">Openbare indruk</h2>
              </div>
              <Badge variant={instructor?.status === "goedgekeurd" ? "success" : "warning"}>
                {instructor?.status ?? "concept"}
              </Badge>
            </div>

            <div className="mt-5 rounded-[1.5rem] border border-slate-200 bg-slate-50/90 p-4 dark:border-white/10 dark:bg-white/5">
              <div className="flex items-start gap-3">
                <div className={`flex size-14 shrink-0 items-center justify-center rounded-[1.2rem] bg-gradient-to-br ${instructor?.profielfoto_kleur ?? instructorColorOptions[0].value} text-sm font-semibold text-white shadow-[0_14px_28px_-20px_rgba(15,23,42,0.42)]`}>
                  {getInitials(profile?.volledige_naam ?? "Instructeur")}
                </div>
              ...
              <p className="mt-4 line-clamp-4 text-sm leading-7 text-slate-600 dark:text-slate-300">
                {instructor?.bio || "Schrijf een korte introductie zodat leerlingen direct snappen hoe jij lesgeeft en waarin je uitblinkt."}
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                {specialisaties.length ? specialisaties.slice(0, 4).map((tag) => (
                  <span key={tag} className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-medium text-slate-700 dark:border-white/10 dark:bg-white/8 dark:text-slate-200">
                    {tag}
                  </span>
                )) : <span className="text-sm text-slate-500 dark:text-slate-400">Nog geen specialisaties toegevoegd.</span>}
              </div>
            </div>
          </section>
          ...
          <section className="rounded-[1.8rem] border border-white/70 bg-white/90 p-5 shadow-[0_26px_86px_-48px_rgba(15,23,42,0.38)] dark:border-white/10 dark:bg-white/6">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-semibold text-slate-950 dark:text-white">Verbeterpunten</h2>
            </div>
            <div className="mt-4 grid gap-2">
              {(improvementTips.length ? improvementTips : ["Je profiel staat er sterk bij. Houd prijs, werkgebied en beschikbaarheid actueel."]).map((tip) => (
                <div key={tip} className="rounded-[1rem] bg-slate-50/85 px-3 py-2.5 text-sm leading-6 text-slate-700 dark:bg-white/6 dark:text-slate-300">
                  {tip}
                </div>
              ))}
            </div>
          ...
          <section className="rounded-[1.8rem] border border-white/70 bg-[linear-gradient(135deg,rgba(255,255,255,0.94),rgba(240,249,255,0.9))] p-5 shadow-[0_26px_86px_-48px_rgba(15,23,42,0.38)] dark:border-white/10 dark:bg-[linear-gradient(145deg,rgba(15,23,42,0.9),rgba(30,41,59,0.84),rgba(15,23,42,0.92))]">
            ...
            <div className="mt-3 grid gap-2">
              {[
                { icon: MapPin, label: "Werkgebied", value: workArea.length ? `${workArea.length} regio's` : "Nog leeg" },
                { icon: WalletCards, label: "Prijs", value: Number(instructor?.prijs_per_les ?? 0) ? formatCurrency(Number(instructor?.prijs_per_les ?? 0)) : "Nog leeg" },
                { icon: BadgeCheck, label: "Transmissie", value: transmissionLabel(instructor?.transmissie) },
              ].map((item) => (
                <div key={item.label} className="flex items-center justify-between gap-3 rounded-[1rem] bg-white/80 px-3 py-2.5 text-sm shadow-[0_14px_34px_-28px_rgba(15,23,42,0.18)] dark:bg-white/6">
                  <span className="inline-flex items-center gap-2 text-slate-600 dark:text-slate-300"><item.icon className="size-4" />{item.label}</span>
